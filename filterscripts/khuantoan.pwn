...
{
GameTextForPlayer(playerid, "~r~Khong duoc tan cong trong khu vuc an toan", 5000, 3);
...
CMD:taokhuantoan(playerid, params[])
{
if(!IsPlayerAdmin(playerid))
{
...
{
SendClientMessage(playerid, -1, "USAGE: /taokhuantoan [ Ten ] [ ID Khuantoan ] [ So tien ]");
...
format(string, sizeof(string), "{FFFFFF}Khu Vuc An Toan(ID: %d)\n{24D12F}%d Meters\n{DB8B35}Khong ban",szid,SafeZoneInfo[szid][szSize]);
...
CMD:xoakhuantoan(playerid, params[])
{
if(!IsPlayerAdmin(playerid))
{
...
new h, string[128];
if(sscanf(params,"d",h)) return SendClientMessage(playerid, -1,"Su dung: /xoakhuantoan [ID Khuantoan]");
